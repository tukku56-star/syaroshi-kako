...
def get_drive_log_dir():
    ...
    try:
        ...
        drive_log_dir = os.path.join(GOOGLE_DRIVE, today, "log")
...
class RoomMonitor:
        ...
        try:
            if "ajax.php" in response.url:
                try:
                    ...
                    print(f"[{self.bot_name}] Received ajax response")
                    await self._save_data(data)
                except Exception as e:
                    print(f"[{self.bot_name}] Error processing ajax response: {e}")
        except:
            pass
    ...
    async def _scrape_and_save_messages(self):
        """Scrape messages directly from DOM and save as simple text"""
        try:
            ...
            safe_room_name = sanitize_filename(self.room_name)
            log_file = os.path.join(drive_dir, f"room_{self.room_id}_{safe_room_name}.txt")
            
            # Get all talk elements from DOM
            talk_elements = await self.page.query_selector_all("#talks .talk")
            
            messages = []
            for elem in talk_elements:
                try:
                    # Extract message data from DOM
                    name_elem = await elem.query_selector(".name")
                    msg_elem = await elem.query_selector(".message")
                    time_elem = await elem.query_selector(".time")
                    
                    name = await name_elem.inner_text() if name_elem else "不明"
                    message = await msg_elem.inner_text() if msg_elem else ""
                    time_str = await time_elem.inner_text() if time_elem else ""
                    
                    if message:
                        messages.append(f"[{time_str}] {name}: {message}")
                except:
                    continue
            ...
            if messages:
                ...
                print(f"[{self.bot_name}] Saved {len(messages)} messages to {os.path.basename(log_file)}")
                self.last_activity = datetime.now()
                
        except Exception as e:
            print(f"[{self.bot_name}] Scrape error: {e}")
    ...
    async def _find_and_enter_unmanned_room(self):
        try:
            occupied_rooms = self.parent_bot.get_occupied_rooms()
            ...
            print(f"[{self.bot_name}] Found {len(room_uls)} room elements")
            ...
            while self.running:
                try:
                    if not self.in_room:
                        ...
                        # Scrape and save messages every 10 seconds
                        await self._scrape_and_save_messages()
